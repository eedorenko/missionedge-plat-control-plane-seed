...
- `applications`
  - `<workspace-name>`
    - `ApplicationRegistrations` - defines the `ApplicationRegistrations` for a given workspace ([sample](/samples/ApplicationRegistration.yaml))
...
- `clusters` - defines the `Clusters` in your platform ([sample](/samples/Cluster.yaml))
- `manifests` - holds Kubernetes YAML for use with `ManifestDeployments`
- `samples` - sample YAML for various Coral entities
- `templates` - defines the available `ApplicationTemplates` in your platform ([sample](/samples/Template.yaml))
- `workspaces` - defines the `Workspaces` in your platform ([sample](/samples/Workspace.yaml))
